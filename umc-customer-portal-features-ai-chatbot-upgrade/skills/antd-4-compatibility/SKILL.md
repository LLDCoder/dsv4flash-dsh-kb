...
description: Review, implement, or debug Ant Design 4.22 compatibility in this customer portal. Use when changing AntD components, adding UI code from current AntD docs, reviewing build/runtime errors, or investigating blank screens involving Dropdown, Menu, Modal, Drawer, Tooltip, Popover, Popconfirm, Select-like dropdowns, Table filters/actions, DatePicker, message/notification APIs, or any suspected AntD 5 API drift in a project pinned to antd@4.22.8.
---
...
```text
Use the local skill at skills/antd-4-compatibility to review this change for AntD 4.22 compatibility.
```

The request can be written in any language. For reliable triggering, include the literal skill path `skills/antd-4-compatibility` or the skill name `antd-4-compatibility`.

For a broader sweep, ask:

```text
Use skills/antd-4-compatibility to scan src for AntD 5-style APIs and fix only real AntD 4.22 compatibility risks.
```
...
2. Scan for common AntD 5 drift patterns.
3. Inspect local component prop types in `node_modules/antd/es/...` for any uncertain API.
4. Fix only compatibility risks that affect the changed area or are clearly unsafe.
5. Preserve existing styling, routes, services, DTOs, action grouping, and business behavior unless the task explicitly asks for broader changes.
6. Re-run the targeted scan after edits.

## Validation

Run the build after compatibility edits:

```bash
npm run build
```

For UI regressions, use the customer browser test flow when the affected page is reachable:

- Log in at `http://localhost:5174/login`.
- Select the relevant establishment profile.

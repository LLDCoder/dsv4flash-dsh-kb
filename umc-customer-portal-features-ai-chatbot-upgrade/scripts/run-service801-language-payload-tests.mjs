...
const tempDirectory = mkdtempSync(join(tmpdir(), "service801-language-tests-"));
const bundlePath = join(tempDirectory, "service801LanguagePayload.test.mjs");
...
try {
  execFileSync(
    esbuildPath,
    [
      "scripts/service801LanguagePayload.test.ts",
      "--bundle",
      "--platform=node",
      "--format=esm",
      "--alias:@=./src",

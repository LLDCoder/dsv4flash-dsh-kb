...
const tempDirectory = mkdtempSync(join(tmpdir(), "id-selector-type-change-tests-"));
const bundlePath = join(tempDirectory, "idSelectorTypeChange.test.mjs");
...
try {
  execFileSync(
    esbuildPath,
    [
      "scripts/idSelectorTypeChange.test.ts",
      "--bundle",
      "--platform=node",
      "--format=esm",

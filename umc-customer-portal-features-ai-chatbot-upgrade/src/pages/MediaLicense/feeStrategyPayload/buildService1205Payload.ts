...
const PARENT_APPLICATION_ID_PATHS = [
  "parentApplicationId",
  "payload.parentApplicationId",
  "payload.applicationContext.parentApplicationId",
  "sourceApplicationId",
  "payload.sourceApplicationId",
  "SelectTable.parentApplicationId",
  "SelectTable.sourceApplicationId",
  "SelectTableSingle.parentApplicationId",
  "SelectTableSingle.sourceApplicationId",
];

const PARENT_LICENSE_ID_PATHS = [
  "parentLicenseId",
  "payload.parentLicenseId",
  "payload.applicationContext.parentLicenseId",
  "sourceMedialLicenseId",
  "sourceMediaLicenseId",
  "payload.sourceMedialLicenseId",
  "payload.sourceMediaLicenseId",
  "SelectTable.parentLicenseId",
  "SelectTable.sourceMedialLicenseId",
  "SelectTableSingle.parentLicenseId",
  "SelectTableSingle.sourceMedialLicenseId",
];
...
const resolveApplicationContext = (
  ...
  if (applicationId === undefined) {
    throw new Error(
      "Unable to build service 1202 fee payload: applicationId is required.",
    );
  }
...
const resolveRequiredLicensePermitNo = (
  ...
  if (!resolved || resolved === "-") {
    throw new Error(
      "Unable to build service 1205 fee payload: licensePermitNo is required.",
    );
  }
...
export const buildService1205PartnerDeltaSummary = (
  formValuesList: Array<Record<string, unknown>>,
) => {
...
const resolveAddedPartnerCount = (
  ...
  if (initialPartnerIds.length > 0) {
    return buildService1205PartnerDeltaSummary(formValuesList).addedCount;
  }
...
const resolveRemovedPartnerCount = (
  ...
  const initialPartnerIds = resolveInitialPartnerIds(formValuesList);
  if (initialPartnerIds.length > 0) {
    return buildService1205PartnerDeltaSummary(formValuesList).removedCount;
  }
...
const resolveRetainedPartnerCount = (
  ...
  const initialPartnerIds = resolveInitialPartnerIds(formValuesList);
  if (initialPartnerIds.length > 0) {
    return buildService1205PartnerDeltaSummary(formValuesList).retainedCount;
  }
...
export const buildService1205FeePayload = async (
  params: BuildServiceFeeStrategyPayloadParams,
): Promise<FeeQuoteEnvelope> => {
  const enginePayload = await buildService1205FeeEnginePayload(params);
...
export const buildService1205FeeEnginePayload = async ({
  config,
  formValuesList,
  currentProfileId,
  userInfo,
  ...
  const { applicationId } = resolveApplicationContext(
    formValuesList,
  );

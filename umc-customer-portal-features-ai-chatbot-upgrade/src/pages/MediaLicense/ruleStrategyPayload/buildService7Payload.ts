...
import type { Service7RuleStrategyValidatePayload } from "@/services/services";
  ...
  return rawTeamMembers
    .map((item) => {
      ...
      return {
        ...
        emiratesId: coerceString(getFirstDefined([get(member, "emiratesId"), get(member, "eid")])),
...
export const buildService7Payload = ({
  config,
  formValuesList,
  currentProfileId,
  userInfo,
  submissionMode = "submit",
}: BuildServiceRuleStrategyPayloadParams): Service7RuleStrategyValidatePayload => {
  ...
  return {
    ...
    request: {
      ...
      formPermitDocumentUrl: resolveUploadUrl(
        getFirstDefined(
          formValuesList.flatMap((formValues) => [
            get(formValues, "FilmingPurpose.formPermitDocumentUrl"),
            get(formValues, "FilmingPurpose.formPermit"),
            get(formValues, "formPermitDocumentUrl"),
            get(formValues, "formPermit"),
            get(formValues, "FormPermit"),
            get(formValues, "Form Permit"),
            get(formValues, "RecentPhotocopyoftheTradeLicense"),
            get(formValues, "Recent Photocopy of the Trade License"),
          ]),
        ),
      ),

...
import type { Service1102RuleStrategyValidatePayload } from "@/services/services";
...
export const buildService1102Payload = async ({
  config,
  formValuesList,
  currentProfileId,
  userInfo,
  submissionMode = "submit",
}: BuildServiceRuleStrategyPayloadParams): Promise<Service1102RuleStrategyValidatePayload> => {
  const selectTableSingle = resolveSelectTableSingleValue(formValuesList);
  if (!selectTableSingle) {
    throw new Error("SelectTableSingle data is missing for service 1102.");
  }
  ...
  if (periodicalTypeId === undefined) {
    throw new Error("Unable to derive service 1102 magazine frequency from selected activity.");
  }

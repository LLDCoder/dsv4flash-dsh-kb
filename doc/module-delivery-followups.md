...
| INS-04 | A vague “What reporting guidance applies to this task?” follow-up had no selected task identifier and incorrectly re-ran the Todo list Tool. | Resolved the no-context case generically: browser E2E routes it to `reference_clarification` with no Tool call. An explicit selected-task hybrid still needs generic multi-evidence orchestration so it can combine a detail Tool with cited knowledge evidence; do not present either source alone as a hybrid answer. |

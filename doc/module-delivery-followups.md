...
| CNT-03 | “How many need attention today?”, urgency drill-down, and 30-day SLA questions are specified but not finally accepted under a Content Manager identity. | The Content Manager browser E2E now routes “Content tasks need attention today” to `admin_content_dashboard_reader` and supplies the inclusive same-day range to the overview Tool. The upstream Admin Portal endpoint returned `502`; the Chatbot reported it without inventing counts. Keep the drill-down and trend cases blocked until that endpoint recovers. |
...
| CNT-05 | The hybrid task-detail plus external-approval guidance conversation is designed but not finally accepted. | Resolved by Content Manager browser E2E. A numbered selection invokes the captured task-detail ID, then a `knowledge.search` restricted to `/umc`. The current fixture's detail payload is authorized but empty; the response states that boundary and separately presents the retrieved FAHR/Itimad External Approval guidance. |
...
| CH-04 | Real E2E evidence now covers Happiness Leader enquiry no-data, refunds and appeals list-to-detail, operational analytics with same-period follow-up, team summary/member, cited guidance, and the write boundary. Happiness Staff returns an authorized empty enquiry result and receives 403 from team summary/task-query endpoints. | The planned selected-enquiry plus knowledge flow is published with explicit ordinal binding. The supplied Leader identity was retested against the page's own list endpoint with omitted/true/false completion variants; every replayable request returned an authorized empty result. Run the hybrid and status/date corrections only when a non-empty API fixture is available. |

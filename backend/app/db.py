...
from sqlalchemy import JSON, Boolean, DateTime, ForeignKey, Index, Integer, String, Text, UniqueConstraint, delete, func, or_, select, text
...
class AuditOperator(Base):
    """A named account for the dedicated customer audit console."""
...
class AuditOperatorSession(Base):
    ...
    __table_args__ = (
        Index("ix_audit_operator_session_expiry", "expires_at", "revoked_at"),
    )
...
async def purge_expired_audit_data(
    ...
    *,
    ...
    """Apply configured retention to conversation and console audit data."""
    ...
    session_result = await session.execute(
        delete(AuditOperatorSession).where(or_(
...
async def init_db() -> None:
    ...
    async with SessionLocal() as session:
        changed = False
        # Never manufacture a deliverable-console credential. Bootstrap is an
        # explicit one-time deployment action and only applies to an empty
        # operator table.

"""Security primitives for the dedicated customer audit console.

Unlike the legacy test console, audit-console sessions are opaque, revocable
database records and operator passwords are never recoverable.
"""
...
def hash_password(password: str, *, iterations: int = PASSWORD_ITERATIONS, salt: bytes | None = None) -> str:
    """Hash an operator password with the standard-library PBKDF2 primitive."""
...
def issue_session_token() -> tuple[str, str]:
    """Return the browser token and the irreversible value stored in the DB."""
